...
const validateCred = (arr) => {
  ...
  let first = reversed.pop();
  // console.log(reversed);
  reversed.reverse();
  // console.log(reversed);

  for (let elem = 0; elem < reversed.length; elem += 2) {
    reversed[elem] *= 2;
    if (reversed[elem] > 9) {
      reversed[elem] -= 9;
    }
  ...
  // console.log(reversed);
  reversed.splice(0, 0, first);
  reversed.reverse();
  // console.log(reversed);
  let sumTotal = reversed.reduce((a, b) => a + b, 0);
...
const idInvalidCardCompanies = (invalids) => {
  const companies = [];
  for (let invalid = 0; invalid < invalids.length; invalid++) {
    switch (invalids[invalid][0]) {
      case 3:
